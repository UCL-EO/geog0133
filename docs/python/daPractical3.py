...
# red
kfr = kalman.Kalman(red,x0_red,C_red,R_red,M,Q_red,H,doys,reverse=True,outlier=3.0)
...
plt.plot(kfr.sample_doys[kfr.bad],kfr.y[kfr.bad],'ro',label='bad obs')
...
# nir
plt.clf()
kfn = kalman.Kalman(nir,x0_nir,C_nir,R_nir,M,Q_nir,H,doys,reverse=True,outlier=3.0)
kfn.run(bp=fireday)
kfn1 = kalman.Kalman(nir,x0_nir,C_nir,R_nir,M,Q_nir,H,doys,reverse=False,outlier=3.0)
kfn1.run(bp=fireday)
...
Z = np.array([(np.matrix(kfn.x[i] - kfn1.x[i]) * \
       (np.matrix(kfn.C[i]).I + np.matrix(kfn1.C[i]).I)* Q_nir *\
       np.matrix(kfn.x[i] - kfn1.x[i]).T)[0,0] \
       for i in range(kfn.x.shape[0])])

# smooth so that only the largest peak remains
f = np.hanning(31)
Zs = np.convolve(f/f.sum(),Z,mode='same')
# enhance so only the major peak remains
Zs2 = Zs**10/(Zs**10).max()

# plot the Z score
plt.plot(kfn.alldoys,Z)
plt.plot(kfn.alldoys,Zs)
